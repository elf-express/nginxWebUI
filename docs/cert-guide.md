...
3. 選範本「Edit zone DNS(編輯區域 DNS)」,權限這樣設:
   - 必要:`Zone → DNS → Edit`(加/刪 TXT 一定要「編輯」權;只有「讀取」會失敗,見排查 #2)
   - 選填:`Zone → Zone → Read`(讓 acme.sh 自動找到 zone。注意「找 zone」是讀取,用 `Read` 不是 `Edit`;步驟 2 若已填 CF_Zone_ID 就不需要這條)
