...
> >$mkdirmy\_project && cdmy\_project
> $ npx許可choose\_your\_license\_here >許可
> $ npx個忽略節點
> 
> $ cat >數據包. json <<EOF
> {
...
> function setbuf（）
> {
>     return set\_buffer(global.hello);
> }
> 
> //調用代碼
> var frame = setbuf();
...
> functionset\_buffer（dnsPacket）
> {
>     //創建DNS數據包字節
>     var buf = dnsPacket.encode({
>         類型：'查詢'，
>         id: 1,
>         flags：dnsPacket.RECURSION\_RED，
>         questions: \[{
>             type: 'A',
>             name：'good.com'
>         }\]
...
> $ cat dist/wp\_out.js code.js > njs\_dns\_bundle.js

讓我們在文件末尾調用代碼：

> var b = set\_buffer(global.dns);
> console.log(b);

並使用node執行它：

> $ node ./njs\_dns\_bundle\_final.js
> Buffer \[Uint8Array\] \[
>     0,   1,   1, 0,  0,   1,   0,   0,
...
> 
>     var resolved\_name = packet.answers\[0\].name;
> 
