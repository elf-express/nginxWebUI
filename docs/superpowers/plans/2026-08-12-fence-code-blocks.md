...
- **不得修改任何程式碼字元。** 唯一允許的變動是剝除 `> ` 前綴、還原 markdown 跳脫，以及加上 fence 標記。
  - 跳脫還原的範圍是 **CommonMark 的可跳脫集合＝全部 32 個 ASCII 標點**（``\!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~``），不是只有 `\_` `\*` `\[` `\]` `` \` ``。理由：引用塊裡 markdown 會吃掉這些反斜線，fence 內不會——沒還原的每一個都會變成畫面上多出來的字元。初版只還原五個字元，導致 23 檔 97 行渲染退步（`\\.` 顯示成 `\\.`、`\#` 顯示成 `\#`），nginx regex 受創最深。
  - 只做**一趟**替換：`\\d` → `\d`、`\\\\d` → `\\d`。做兩趟會把 C／Perl 字串裡本來就該留著的反斜線再吃掉一次。
  - `unescapeMd` 與 `contentFingerprint` 的 `FP_ESCAPE_RE` 是刻意的複本，必須逐字元同步；只改一邊時指紋會當場報「內容指紋不符」。
