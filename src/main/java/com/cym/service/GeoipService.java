...
public class GeoipService {
	...
	/**
	 * 啟動保底:realip.conf 不存在時產生「僅內網信任來源」的最小版。
	 * InitConfig 種子的 http 參數 include 此檔且屬 LOCKED_GROUPS(不可取消勾選),
	 * 檔案缺失會讓 nginx -t 永遠失敗 → saveEnable 全部 rollback(使用者死鎖)。
	 * Cloudflare 範圍留待 downloadCloudflare()/update-geoip-cf.sh 覆寫。
	 */
	public void ensureRealipPlaceholder() {
		try {
			File dir = new File(GEOIP_DIR);
			...
			if (!dir.exists() && !dir.mkdirs()) {
				logger.warn("GeoIP 目錄無法建立,略過 realip.conf 保底: {}", GEOIP_DIR);
				return;
			}
			StringBuilder sb = new StringBuilder();
			sb.append("# Cloudflare Real IP - placeholder generated by nginxWebUI (download Cloudflare IP to replace)\n\n");
			sb.append("# Local / Docker / Private Network Trust\n");
			...
			FileUtil.move(tmp, dest, true);
			logger.info("realip.conf 保底已建立(僅內網信任來源): {}", dest.getAbsolutePath());
		} catch (Exception e) {
			logger.warn("realip.conf 保底建立失敗(不影響啟動): {}", e.getMessage());
		}
	}
